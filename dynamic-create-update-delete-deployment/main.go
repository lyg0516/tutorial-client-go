...
func main() {
	...
	//
	//	1. Modify the "deployment" variables and call: Update(deployment).
	//		This works like the "kubectl replace" command and it overwrites/loses changes
	//		mad by other clients between you Create() and Update() the object.
	//  2. Modify the "result" returned by Get() and retry Update(result) until
	//     you no longer get a conflict error. This way, you can preserve changes made
	//	   by other clients between Create() and Update(). This is implemented below
	//	   using the retry utility package included when client-go (RECOMMENDED)
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if err != nil || !found || containers == nil {
			panic(fmt.Errorf("Failed to find template containers in deployment: %v", result.GetName()))
		}

		// update container[0] image
		if err := unstructured.SetNestedField(containers[0].(map[string]interface{}), int64(1), "spec", "template", "spec", "containers"); err != nil {
			panic(err)
		}
	...
	for _, d := range list.Items {
		...
		if err != nil || !found || replicas == 0 {
			fmt.Printf("Replicas not found for deployment %s: error=%s", d.GetName(), err.Error())
			continue
		}
		fmt.Printf("Replicas found for deployment %s: replicas=%d", d.GetName(), replicas)
	}
